...
data.dir <- file.path(main.dir, "Data/OUwie/")
...
Afr_avgl <- read.table(file.path(data.dir, "issues_AVGL.txt"), header=T)
...
conv_Fail <- Africa %>% filter(dat == "empirical") %>%
...
pdf(file.path(fig.dir,"Fig0_ProportionConvFail.pdf"),width=5, height=3)
gridExtra::grid.arrange(conv_Fail)
dev.off()
