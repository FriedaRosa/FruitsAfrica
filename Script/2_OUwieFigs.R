...
data.dir <- file.path(main.dir, "Data/OUwie/out/")
...
# 100 trees results:
Afr_emp <- read.table(file.path(data.dir,"merged_data_AVGL.txt"), header=T)

# MCC results:
MCC <- read.table(file.path(data.dir,"merged_MCC_all.txt"), header=T)
...
Afr_merged <- merge(Afr_emp, MCC_afr, all=T)
...
write.csv(sum_tab, file.path(res.dir, "OUwie_SumTab_AfrOnly.csv"))
...
pdf(file.path(fig.dir,"Fig1_Count_selectedModels_grid.pdf"),width=5, height=3)
gridExtra::grid.arrange(p2_1)
dev.off()
...
pdf(file.path(fig.dir,"Fig2_OUwie_dotlineplots.pdf"),width=5, height=6)
