...
dd$max95_BBM <- exp(dd$log_max95FL_palms_BBM_MCC)
dd$sqrt(exp(log_max95FL_palms_BBM_MCC))  <- sqrt(exp(dd$log_max95FL_palms_BBM_MCC))
...
pdf(file.path(fig.dir, "Anova_emp.pdf"), width = 6, height = 4)
...
FL_violin <- FL_violin + 
  #labs(subtitle = expression(paste("Africa:"~beta~"= 0.24, se = 0.11, p = 0.036"))) + 
  theme(legend.position="none", plot.subtitle = element_text(vjust = -7, hjust=0.1)) 
FL_violin
dev.off()
...
maxFS_p <- ggplot() +
  ...
  geom_point(aes(x = LONG, y = LAT,
             ...
             data =tdwg_maxFS, alpha = 0.9,  pch = 21, colour = "black") +
  coord_fixed() +
  ...
  theme_classic()+
   theme(
    ...
    plot.margin = margin(t = 12,  # Top margin
                         ...
                         l = 12)) # Left margin) #change font size of legend title   
#  Add p-value
pdf(file.path(fig.dir, "Anova_sim.pdf"), width = 6, height = 4)
...
sim_violin <- sim_violin + 
  #labs(subtitle = expression(paste("Africa:"~beta~"= -0.04, se = 0.11, p = n.s."))) + 
  theme(legend.position="none", plot.subtitle = element_text(vjust = -6, hjust=0.015)) +coord_fixed(ratio=3.25)   #  Add p-value
sim_violin
dev.off()
...
maxFSsim_p_new <- ggplot() +
  ...
  coord_fixed() +
  map_theme + theme(plot.title = element_blank()) +
  guides(size="none", fill = guide_legend(title = "Maximum\n(95th percentile)\nfruit length [cm]", override.aes = list(size = size_v))) +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0)) +
...
ggsave(file.path(fig.dir, "3_palms_meanBSchange_map.pdf"), abs_decr_BS_map, width = 8, height = 4, device = "pdf")
...
percBSchange <- ggpubr::ggviolin(dd, x = "accAfrica", y = "mean_perc_BSchange", fill="accAfrica", color ="#636363", show.legend=F, trim = T) +
  ...
  theme(
    ...
    plot.margin = margin(t = 12,  # Top margin
                         ...
                         l = 12)) # Left margin) #change font size of legend title   
#  Add p-value
pdf(file.path(fig.dir, "Anova_percBSchange_binary.pdf"), width = 6, height = 4)
...
ggsave(file.path(fig.dir, "3_palms_mean_percBSchange_map.pdf"), perc_decr_BS_map, width = 8, height = 4, device = "pdf")
...
plotNormalDensity(sqrt(exp(tdwg_final3$log_max95FL_palms_BBM_MCC))) #?
plotNormalDensity(tdwg_final3$log_max95FL_palms_BBM_MCC) #?
...
p0bx <- ggplot(tdwg_final_0, aes(factor(accAfrica), sqrt(exp(log_max95FL_palms_BBM_MCC)) )) +
  geom_violin(aes(fill=accAfrica, col=accAfrica)) +
  scale_fill_manual(values=my_colors, limits = c("1", "0"), labels=c('Africa', 'Elsewhere')) +
  scale_color_manual(values=my_colors, limits = c("1", "0"), labels=c('Africa', 'Elsewhere'))+
  theme_classic()+  
...
p0b <- ggplot(tdwg_final_0, aes(factor(accAfrica), sqrt(exp(log_max95FL_palms_BBM_MCC)) )) +
...
p1b <- ggplot(tdwg_final_0, aes(sqrt_change, sqrt(exp(log_max95FL_palms_BBM_MCC)) )) +
  geom_point(aes(color=accAfrica), show.legend = F) +
  stat_smooth(method = lm, col="#636363")+
  scale_color_manual(values=my_colors,limits = c("1", "0"), labels=c("0" ='Elsewhere', "1" = 'Africa')) +
  theme_classic()+
...
p2b <- ggplot(tdwg_final_0, aes(sqrt(curr_max95BodySize), sqrt(exp(log_max95FL_palms_BBM_MCC)) )) +
  geom_point(aes(color=accAfrica), show.legend = F) +
  #stat_smooth(method = lm, col="#636363")+
  scale_color_manual(values=my_colors,limits = c("1", "0"), labels=c("0" ='Elsewhere', "1" = 'Africa')) +
  theme_classic()+ 
...
## do not use in main text: 
p3b <- ggplot(tdwg_final_0, aes((MAT)^2, sqrt(exp(log_max95FL_palms_BBM_MCC)) )) +
  geom_point(aes(color=accAfrica), show.legend = F) +
  #stat_smooth(method = lm, col="#636363")+
  scale_color_manual(values=my_colors, labels=c('Elsewhere', 'Africa')) +
  theme_classic()+
...
p4b <-ggplot(tdwg_final_0, aes(sqrt(TempSeas),sqrt(exp(log_max95FL_palms_BBM_MCC)) )) +
  geom_point(aes(color=accAfrica), show.legend = F) +
  stat_smooth(method = lm, col="#636363")+
  scale_color_manual(values=my_colors, labels=c('Elsewhere', 'Africa')) +
  theme(legend.position="none")+
...
p5b <- ggplot(tdwg_final_0, aes(sqrt(AnnualPrec), sqrt(exp(log_max95FL_palms_BBM_MCC)) )) +
  geom_point(aes(color=accAfrica), show.legend = F) +
  stat_smooth(method = lm, col="#636363")+
  scale_color_manual(values=my_colors, labels=c('Elsewhere', 'Africa')) +
  theme_classic()+
...
p6b <- ggplot(tdwg_final_0, aes(sqrt(PrecSeas), sqrt(exp(log_max95FL_palms_BBM_MCC)) )) +
  geom_point(aes(color=accAfrica), show.legend = F) +
  #stat_smooth(method = lm, col="#636363")+
  scale_color_manual(values=my_colors, labels=c('Elsewhere', 'Africa')) +
  theme(legend.position="none")+
...
p7b <- ggplot(tdwg_final_0, aes(sqrt(CH_Mean), sqrt(exp(log_max95FL_palms_BBM_MCC)) )) +
  geom_point(aes(color=accAfrica), show.legend = F) +
  #stat_smooth(method = lm, col="#636363")+
  scale_color_manual(values=my_colors, labels=c('Elsewhere', 'Africa')) +
  theme_classic()+
...
pdf(file.path(fig.dir, "allPlotsLMsFormulasSimulated.pdf"), height = 12, width = 12.75)
...
pdf(file.path(fig.dir, "finalPlotLMsFormulas.pdf"), width = 16, height = 5)
x<- cowplot::plot_grid(FL_violin, p2a, p1a, legend, nrow=1, labels=c('a)','b)', 'c)'), align = "hv",axis = c("lbt"))
x +  theme(plot.margin = margin(12, 12, 12, 12))
dev.off()

pdf(file.path(fig.dir, "finalPlotLMsFormulasSimulated.pdf"), width = 16, height = 5)
...
lm_full1b <- lm(normalized(sqrt(exp(log_max95FL_palms_BBM_MCC)) ) ~ factor(accAfrica) +
                 normalized((sqrt_change)) + 
                 normalized(sqrt(curr_max95BodySize)) + 
                 normalized(I(MAT^2)) +
                 normalized(sqrt(TempSeas)) + 
               ...
               data = full_df, na.action = na.exclude)
summary(lm_full1b)
plot(normalized(sqrt(exp(log_max95FL_palms_BBM_MCC)) ) ~ 
       normalized((sqrt_change)), data = full_df)

plot_model(lm_full1b, col= cols, type= "est", show.intercept = F, show.values = T, show.p = T, title = "Maximum fruit length \n(Palms) w/o interactions", axis.labels = c("Canopy height", "PrecSeas", "Annual Prec", "TempSeas", "MAT", "body mass", "body mass decrease", "Distribution in Africa"))


lm_full2b <- lm(normalized(sqrt(exp(log_max95FL_palms_BBM_MCC)) ) ~ 
                 normalized((sqrt_change))*factor(accAfrica) + 
                 normalized(sqrt(curr_max95BodySize))*factor(accAfrica) + 
                 normalized(I(MAT^2)) +
                 normalized(sqrt(TempSeas)) + 
