...
for (i in 1:length(tree)){
  asia <- ReorderData(tree[[i]], asia, taxa.names="names")
}
for (i in 1:length(tree)){
...
weights <- list()
for (i in 1:length(tree)){
  equal <- fitDiscrete(tree[[i]], asia, model = "ER")
  sym <- fitDiscrete(tree[[i]], asia, model = "SYM")
  ard <- fitDiscrete(tree[[i]], asia, model = "ARD")
...
save.image("mk_models_asia_22Sep2022.rda")
