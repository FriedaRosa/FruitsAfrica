...
# model fitting of Mk models revealed best support for ER model, however 
# biologically an ARD model makes more sense and was prefered here;
# aicw was > 0.6 for ARD 
...
## Africa x avgl
data_Afr_avgl <- data.frame(species=names(avgl), regime = africa, trait = as.data.frame(avgl))
data_Afr_avgl$species <- as.character(data_Afr_avgl$species)
data_Afr_avgl$regime <- as.factor(data_Afr_avgl$regime)
data_Afr_avgl$avgl <- as.numeric(as.character(data_Afr_avgl$avgl))
