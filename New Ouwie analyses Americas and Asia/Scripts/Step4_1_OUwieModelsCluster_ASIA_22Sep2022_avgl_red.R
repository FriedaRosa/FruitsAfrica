...
# selecting columns and removing duplicate  d species rows
...
## Asia / non-Asia
asia <- numeric(length = nrow(data2))
names(asia) <- rownames(data2)
asia[data2[,"Asia"]=="0"] <- "1_world"
asia[data2[,"Asia"]=="1"] <- "2_asia"
...
asia <- ReorderData(tree, asia, taxa.names="names")
...
## Asia
simmap_asia <- make.simmap(tree, asia, model="ARD", nsim=100, pi=c(1,0))

# summarize 100 simulations from simmap
x <- getStates(simmap_asia,"nodes")
sumr <- apply(x, 1, function(x){
  mfv <- table(x)
  return(names(mfv[mfv==max(mfv)]))
})
simmap_asia[[1]]$node.label <- sumr
simmap_ASIA <- simmap_asia[[1]]
...
## Asia x BBM:
#data_Asia_BM <- data.frame(species=data2$SpecName, regime = asia, trait = data2$log_BBM_MCC)
#data_Asia_BM$species <- as.character(data_Asia_BM$species)
#data_Asia_BM$regime <- as.factor(data_Asia_BM$regime)
#data_Asia_BM$trait <- as.numeric(as.character(data_Asia_BM$trait))
#str(data_Asia_BM)

## Asia x avgl
data_Asia_avgl <- data.frame(species=data2$SpecName, regime = asia, trait = data2$log_BBM_mean)
data_Asia_avgl$species <- as.character(data_Asia_avgl$species)
data_Asia_avgl$regime <- as.factor(data_Asia_avgl$regime)
data_Asia_avgl$trait <- as.numeric(as.character(data_Asia_avgl$trait))
print(str(data_Asia_avgl))

# run OUwie models ===================================

## Asia x avgl
BM_Asia_avgl <- OUwie(simmap_ASIA, data_Asia_avgl, model="BM1", simmap.tree = TRUE, diagn=TRUE)
BMS_Asia_avgl <- OUwie(simmap_ASIA, data_Asia_avgl, model="BMS", simmap.tree = TRUE, diagn=TRUE)
OU1_Asia_avgl <- OUwie(simmap_ASIA, data_Asia_avgl, model="OU1", simmap.tree = TRUE, diagn=TRUE)
OUM_Asia_avgl <- OUwie(simmap_ASIA, data_Asia_avgl, model="OUM", simmap.tree = TRUE, diagn=TRUE)
OUMV_Asia_avgl <- OUwie(simmap_ASIA, data_Asia_avgl, model="OUMV", simmap.tree = TRUE, diagn=TRUE)
OUMA_Asia_avgl <- OUwie(simmap_ASIA, data_Asia_avgl, model="OUMA", simmap.tree = TRUE, diagn=TRUE)
OUMVA_Asia_avgl <- OUwie(simmap_ASIA, data_Asia_avgl, model="OUMVA", simmap.tree = TRUE, diagn=TRUE)
# save output files
ouwie_Asia_avgl <- list(BM_Asia_avgl, BMS_Asia_avgl, OU1_Asia_avgl, OUM_Asia_avgl, OUMV_Asia_avgl, OUMA_Asia_avgl, OUMVA_Asia_avgl)


## Asia x BBM:
#BM_Asia_BM <- OUwie(simmap_ASIA, data_Asia_BM, model="BM1", simmap.tree = TRUE, diagn=TRUE)
#BMS_Asia_BM <- OUwie(simmap_ASIA, data_Asia_BM, model="BMS", simmap.tree = TRUE, diagn=TRUE)
#OU1_Asia_BM <- OUwie(simmap_ASIA, data_Asia_BM, model="OU1", simmap.tree = TRUE, diagn=TRUE)
#OUM_Asia_BM <- OUwie(simmap_ASIA, data_Asia_BM, model="OUM", simmap.tree = TRUE, diagn=TRUE)
#OUMV_Asia_BM <- OUwie(simmap_ASIA, data_Asia_BM, model="OUMV", simmap.tree = TRUE, diagn=TRUE)
#OUMA_Asia_BM <- OUwie(simmap_ASIA, data_Asia_BM, model="OUMA", simmap.tree = TRUE, diagn=TRUE)
#OUMVA_Asia_BM <- OUwie(simmap_ASIA, data_Asia_BM, model="OUMVA", simmap.tree = TRUE, diagn=TRUE)
# save output files
#ouwie_Asia_BM <- list(BM_Asia_BM, BMS_Asia_BM, OU1_Asia_BM, OUM_Asia_BM, OUMV_Asia_BM, OUMA_Asia_BM, OUMVA_Asia_BM)
...
### choose: 
ouwie1 <- ouwie_Asia_avgl
# ouwie1 <- ouwie_Asia_BM 
